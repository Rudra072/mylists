...
function Welcomescreen() {
  ...
  return (
    <section className="bg-Background100 grid grid-cols-10 min-h-[100vh]">
      <div
        className="absolute w-full h-full bg-center bg-cover -z-10 blur-[1px] fade"
        style={{ backgroundImage: imagePreview && `url(${imagePreview})` }}
      />
      <div className="col-span-6 px-[150px] py-[50px]">
        <div className="mb-6">
          <Logo />
          <h1 className="text-1000 text-headingH2 font-bold leading-[1.2] -tracking-[.3px]">
            Welcome to My List! Let's create your workspace.
          </h1>
        </div>
        <WorkspaceForm image={image} />
      </div>
      {/* <div
        className={`${
          image ? "bg-none" : "bg-Blue"
        }  col-span-4 flex flex-col justify-end items-center px-[150px] py-[80px]`}
      >
        <div className="flex bg-white w-min items-center gap-5 rounded-[10px] px-[13px] py-[10px]">
          <svg
            width="40"
            height="40"
            viewBox="0 0 24 24"
            className="bg-200 rounded-[7px] p-1"
            fill="none"
