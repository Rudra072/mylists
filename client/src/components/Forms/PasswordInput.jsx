...
const PasswordInput = ({ text, onChange, onBlur, value, name, error }) => {
  const [isVisible, setIsVisible] = useState(false);

  return (
    <div>
      <h1 className="text-titleMD font-bold text-1000 mb-1">{text}</h1>
      <div className="relative">
        <input
          ...
          className="bg-white border-[1.5px] w-full py-[8px] px-[20px] border-300 text-1000 rounded-[10px] outline-none focus:ring-2 focus:ring-Blue"
        />
        ...
        <svg
          ...
          className={`absolute inset-y-0 right-[5.5%] top-[23%] cursor-pointer ${
            !isVisible && "hidden"
          }`}
        ...
        <svg
          ...
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={`absolute inset-y-0 right-[5.5%] top-[20%] cursor-pointer ${
            isVisible && "hidden"
          }`}
